...
/**
 * Servlet di creazione grafici a barre orizzontali
 */
public class HorizontalBarChartServlet extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String strXlabel = session.getAttribute("xLabel").toString();
		String strYlabel = session.getAttribute("yLabel").toString();
		String[] labelYAxis = (String[])session.getAttribute("labelXAxis");
		...
		int max_month = labelYAxis.length;
		...
		XAxis.scale.min = 0;
		XAxis.scale.max = max_value;
		YAxis.scale.min = 0;
		//YAxis.scale.max = labelYAxis.length;
		...
		YAxis.scale.max = ampMonthAxis;
		
		YAxis.IntegerScale = true;
		XAxis.IntegerScale = true;
		YAxis.scaleTickInterval = 1;
                //XAxis.scaleTickInterval = XAxis.scale.max/8;
		XAxis.scaleTickInterval = XAxis.scale.max/15;
                // Griglia orizzontale
		...
		HAxisLabel XLabel = new HAxisLabel(strXlabel,java.awt.Color.black,new Font("Arial",Font.BOLD,15));
		VAxisLabel YLabel = new VAxisLabel(strYlabel,java.awt.Color.black,new Font("Arial",Font.BOLD,15));
		chart.XLabel = XLabel;
		chart.YLabel = YLabel;

		// labels dei mesi sull'asse y (01=Jan, 02=Feb, ...)
		YAxis.tickLabels = labelYAxis;

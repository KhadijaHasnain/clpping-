<%@page contentType="text/html" import="java.util.Vector,java.util.Hashtable,it.vidiemme.clipping.utils.*,it.vidiemme.clipping.beans.UserBean,it.vidiemme.clipping.beans.DivisionBean"%>
...
// Recupera l'id della division da modificare
...
if(request.getAttribute("DIVISION_BEAN") == null && !divisionId.equals("")){
	// è in modifica ma non è passato dalla servlet
	divisionBean = new DivisionBean(divisionId);
}else if(request.getAttribute("DIVISION_BEAN") != null){
	// è in modifica ed è passato dalla servlet
	divisionBean = (DivisionBean)request.getAttribute("DIVISION_BEAN");
}
// Ottiene dato l'id ad essa relativo, il nome dell'area a cui appartiene la division da modificare
String areaDescription = DbUtils.getAreaDescription(divisionBean.getArea_id());
String area_id = (request.getParameter("area_id")==null)?divisionBean.getArea_id():request.getParameter("area_id");
	...
	<TR>
		<TD class="td_content">
			<TABLE cellpadding="0" cellspacing="15" border="0" width="100%">
				...
				<TR>
					<TD class="text_13_blu"><B>DIVISIONS - MODIFY DIVISION</B></TD>
				</TR>
				<TR>
					<TD>
						<FORM name="update_form" action="<%=request.getContextPath()%>/manageDivision.do" method="POST">
						<INPUT type="hidden" name="operation" value="update">
						<INPUT type="hidden" name="division_id" value="<%=divisionBean.getDivision_id()%>">
						<INPUT type="hidden" name="pagina" value="<%=pagina%>">
						<INPUT type="hidden" name="group" value="<%=group%>">
						<INPUT type="hidden" name="area_id_search" value="<%=area_id_search%>">
						<INPUT type="hidden" name="name_search" value="<%=Utils.formatStringForView(name_search)%>">
								...
								<TD>
									<TABLE width="70%" border="0" cellspacing="0" cellpadding="6" align="center">
										<TR>
											...
											<%}else{%>
												<%=areaDescription%>
												<INPUT type="hidden" name="area_id" value="<%=divisionBean.getArea_id()%>">
											<%}%>
											</TD>
										</TR>
										<TR>
											<TD class="text_12_black"><B>Name*</B></TD>
											<TD><INPUT type="text" name="name" value="<%=Utils.formatStringForView(divisionBean.getName())%>" class="input_text" maxlength="60"></TD>
										</TR>
										<TR>
											<TD class="text_12_black"><B>Description</B></TD>
											<TD>
												<TEXTAREA name="description" rows="5" cols="50"><%=divisionBean.getDescription()%></TEXTAREA>
											</TD>
										</TR>
										<TR>
											...
											<TD>
												<SELECT name="status" class="select">
													<OPTION value=""></OPTION>
													<OPTION value="1" <%=(divisionBean.getStatus().equals("1"))?"selected":""%>>Archived</OPTION>
													<OPTION value="0" <%=(divisionBean.getStatus().equals("0"))?"selected":""%>>Not Archived</OPTION>
												</SELECT>
											</TD>
										</TR>
										<TR>
